...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    sudo pacman -S hyprland dms-shell-hyprland xdg-desktop-portal-hyprland \
        ...
        alsa-utils pacman-contrib zoxide fzf atuin eza starship fd k9s kubectx stern lazygit watchexec lazydocker xh upower power-profiles-daemon
    [ $? -ne 0 ] && exit 1
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    yay -S catppuccin-cursors-mocha papirus-folders greetd-dms-greeter-bin dracula-gtk-theme forgit
